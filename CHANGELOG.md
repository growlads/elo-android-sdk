...
- **In-chat card layout.** The card is now set like a feed ad: a header row
  ...
  scrolling), and the call-to-action pill on a row of its own at the end
  edge. The card is taller than before and its height depends on the copy,
  so a bounded slot now needs a `maxHeight` of at least 192dp to get it;
  shorter slots get the strip, as does the loading placeholder.
  `EloAdStyle.descriptionOverflow` now applies to the strip only.
- **New: `EloDebugEvent.AdOperation.tapSurface`.** The operation reports the
  ad's `presentation.tap_surface` value (`cta` or `full`) next to the other
  presentation values; `null` when not sent or on a no-fill. Kotlin callers
  that use `AdOperation.copy` or destructure it need to recompile.
